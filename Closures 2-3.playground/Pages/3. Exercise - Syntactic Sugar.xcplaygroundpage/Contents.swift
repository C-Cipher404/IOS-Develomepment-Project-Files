...
let sortedCollection3 = classmates.sorted { (name1: String, name2: String) -> Bool in
    return name1 < name2
}
...
let sortedCollection4 = classmates.sorted {
    $0 < $1
}
...
// Swift knows the collection contains Strings, so it figures out the closure parameter types automatically.
...
let reversedCollection = classmates.sorted {
    $0 > $1
}
